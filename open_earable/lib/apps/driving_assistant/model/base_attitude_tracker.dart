...
abstract class BaseAttitudeTracker {
  StreamController<DrivingAttitude> _drivingAttitudeStreamController =
      StreamController<DrivingAttitude>.broadcast();
  ...
  void updateDrivingAttitude(
    ...
    if (roll == null &&
      ...
      throw ArgumentError(
          "Either roll, pitch, yaw and gyroY or DrivingAttitude must be provided");
    }
    // Check if DrivingAttitude is not null, otherwise use the angles
    drivingAttitude ??= DrivingAttitude(
        roll: roll ?? 0, pitch: pitch ?? 0, yaw: yaw ?? 0, gyroY: gyroY ?? 0);
