...
// Definitions
class MadgwickAHRS {
  double sampleFreq = 512.0; // sample frequency in Hz
  double betaDef = 0.1; // 2 * proportional gain
  late double beta;
  //---------------------------------------------------------------------------------------------------
  ...
  List<double> get quaternion => [q0, q1, q2, q3];
  ...
  void update(double ax, double ay, double az, double gx, double gy, double gz,
    ...
    if (!((ax == 0.0) && (ay == 0.0) && (az == 0.0))) {
      ...
      recipNorm = invSqrt(
          s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
      s0 *= recipNorm;
      s1 *= recipNorm;
      s2 *= recipNorm;
      s3 *= recipNorm;
  ...
  void updateIMU(double ax, double ay, double az, double gx, double gy,
      double gz, double dt) {
    double recipNorm;
    double s0, s1, s2, s3;
    double qDot1, qDot2, qDot3, qDot4;
