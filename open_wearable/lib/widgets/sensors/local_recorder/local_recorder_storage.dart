...
Future<Directory> getAppleRecordingsDirectory() async {
...
Future<Directory?> getRecordingsRootDirectory() async {
  ...
  if (Platform.isIOS || Platform.isMacOS) {
    return getAppleRecordingsDirectory();
  }
  return null;
}
...
Future<String?> pickRecordingDirectory() async {
  ...
  if (Platform.isIOS || Platform.isMacOS) {
    final recordingName =
        'OpenWearable_Recording_${DateTime.now().toIso8601String()}';
    final dirPath =
        '${(await getAppleRecordingsDirectory()).path}/$recordingName';
    final dir = Directory(dirPath);
    if (!await dir.exists()) {
      await dir.create(recursive: true);
    }
